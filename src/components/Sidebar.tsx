...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, isOpen, onClose }) => {
  ...
  const navItems: { id: ViewType; labelKey: string; icon: React.ElementType; color: string }[] = [
    { id: 'dashboard', labelKey: 'navigation.dashboard', icon: LayoutDashboard, color: 'text-blue-600' },
    { id: 'profile', labelKey: 'navigation.profile', icon: User, color: 'text-indigo-600' },
    { id: 'medications', labelKey: 'navigation.medications', icon: Pill, color: 'text-emerald-600' },
    { id: 'lab_tests', labelKey: 'navigation.labTests', icon: FlaskConical, color: 'text-amber-600' },
    { id: 'radiology', labelKey: 'navigation.radiology', icon: ScanLine, color: 'text-purple-600' },
    { id: 'diagnoses', labelKey: 'navigation.diagnoses', icon: Stethoscope, color: 'text-rose-600' },
    { id: 'surgeries', labelKey: 'navigation.surgeries', icon: Scissors, color: 'text-cyan-600' },
    { id: 'timeline', labelKey: 'navigation.timeline', icon: Clock, color: 'text-orange-600' },
    { id: 'share', labelKey: 'navigation.share', icon: Share2, color: 'text-teal-600' },
  ];
  return (
    <>
      {/* Mobile overlay */}
      ...
      >
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider px-3 mb-2">{t('common.menu') || 'Menu'}</p>
          {navItems.map((item) => {
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-3">
            ...
            <p className="text-[10px] text-blue-600/70">{t('common.secureData') || 'Your data is encrypted and secure'}</p>
          </div>
        </div>
      </aside>
    </>
